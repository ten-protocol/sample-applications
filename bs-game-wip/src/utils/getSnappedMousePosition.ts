import {HEX_HEIGHT, HEX_WIDTH, VERTICAL_OFFSET, HEX_GRID_MARGIN} from "@/lib/constants";

export default function getSnappedMousePosition (x: number, y: number) {
    const snappedY = (Math.round(y / (HEX_HEIGHT - VERTICAL_OFFSET)) * (HEX_HEIGHT - VERTICAL_OFFSET)) + (HEX_GRID_MARGIN/4);
    let snappedX = 0
