...
export default function MessageLog() {
  ...
  return (
    <HudWindow headerTitle={currentIndex < messages.length || !readyForNewLine ? "Incoming message...." : "Message Log"}>
        ...
        <ul className="flex flex-col-reverse justify-start">
          {displayedItems.map((message, i) => (
              <li className={styles.line} key={message.id + i}>
                <AnimatedText className="text-sm" text={message.text} onComplete={handleCompletedLine}/>
              </li>
          ))}
            {readyForNewLine && <li><span className={`${styles.cursor} ${styles.withFlash}`}/></li>}
        </ul>
      </div>
    </HudWindow>
  );
