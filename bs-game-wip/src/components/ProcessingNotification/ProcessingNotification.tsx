...
export default function ProcessingNotification () {
    const [guessState, lastError, resetGuessState] = useContractStore((state) => [state.guessState, state.lastError, state.resetGuessState])
    ...
    if (guessState === "STARTED") {
        ...
        bodyContent = <div className="flex flex-col items-start">
            <p className="text-lg bg-stone-900 inline-block px-1">Target locked and confirmed.</p>
    ...
    if (guessState === 'TRANSACTION_SUCCESS') {
        ...
        bodyContent = <div className="flex flex-col items-start">
            <p className="text-lg bg-neutral-900 inline-block px-1">Ordinance en route to target.</p>
            <p className="text-sm bg-neutral-900 inline-block px-1">Monitor for impact confirmation.</p>
        </div>
    }
