...
import {HEX_HEIGHT, HEX_WIDTH, hexHitArea, VERTICAL_OFFSET} from "@/lib/constants";
import {useContractStore} from "../../stores/contractStore";

export default function BattleGridCursor () {
    const [[x,y], hoveredCell] = useBattleGridStore((state) => [state.mousePosition, state.hoveredCell])
    ...
    useTick((delta) => {
        if (shapeRef.current) {
            ...
            const maxScale = 1
            const minScale = isLoadingState ? .7 : .9
