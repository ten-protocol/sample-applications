...
export default function Graveyard () {
    const [graveyard, getGraveyard, hits] = useContractStore(state => [state.graveyard, state.getGraveyard, state.hits])
    ...
    const Footer = <div className="text-right">
        ...
        <h3 className="text-3xl mt-4">{sunkShipTotal}/{graveyard.length}</h3>
        <h6 className="text-sm">Ships remaining</h6>
    </div>

    return (
        <HudWindow headerTitle="Enemy Fleet" isOpen={isConnected} footerContent={Footer} closedContent={Disconnected}>
            <div className="grid grid-cols-10 gap-1 my-2">
                {graveyard.map((ship, i) => <img key={i} src={ship ? ShipSunk : ShipFresh} alt="Logo" width="20" height="10" /> )}
            </div>
        </HudWindow>
    )
}