  ...
  if (allBoardBlocks.value) {
    ...
    if (isHorizontal) {
      valid = shipBlocks.every(
        (_shipBlock, index) =>
          Number(shipBlocks[0].id) % blockWidth !== blockWidth - (shipBlocks.length - (index + 1))
      )
    } else {
      valid = shipBlocks.every(
        (_shipBlock, index) => Number(shipBlocks[0].id) < 90 + (blockWidth * index + 1)
      )
    }
...
function dropShip(e: DragEvent, i: string) {
  ...
  if (data && e.target instanceof HTMLElement) {
    const draggedShipElement = document.getElementById(data) as HTMLElement | null
    ...
    if (ship) {
      ...
      for (let i = 0; i < ship.length; i++) {
        const currentId = isVertical ? startId + i * width.value : startId + i
  ...
  if (cpuBlockRef.value) {
    ...
    if (block?.classList.contains('taken')) {
      ...
      battleships.forEach((ship) => {
        const count = cpuHitShips.value.filter((shipType) => shipType === ship.shipType).length
...
function cpuTurn() {
  ...
  if (playerBlockRef.value) {
    const playerBlocks = Array.from(playerBlockRef.value as any) as HTMLElement[]

    let randomStartIndex = Math.floor(Math.random() * blockWidth * blockWidth)
    const block = playerBlocks.find((block) => block.id === String(randomStartIndex))
    ...
    if (block?.classList.contains('taken')) {
      ...
      battleships.forEach((ship) => {
        const count = playerHitShips.value.filter((shipType) => shipType === ship.shipType).length
        if (count === ship.length) {
          if (playerSunkShips.value.some((x) => x.shipType === ship.shipType)) {
            return
          }
