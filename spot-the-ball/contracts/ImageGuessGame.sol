...
contract ImageGuessGame {
  ...
  function declareWinnersAndDistributePrizes(uint256 _challengeId) private {
    ...
    uint256[3] memory prizeDistribution = [
      (totalPrizePool * 60) / 100,
      (totalPrizePool * 30) / 100,
      (totalPrizePool * 10) / 100
    ];
    ...
    for (uint8 i = 0; i < topGuesses[_challengeId].length; i++) {
      if (topGuesses[_challengeId][i].guesser != address(0)) {
        ...
        emit ChallengeWinner(
          _challengeId,
          topGuesses[_challengeId][i].guesser,
          prizeDistribution[i],
          i + 1
        );
      }
  ...
  function updateTopGuesses(uint256 _challengeId, GuessDetails memory newGuess) private {
    ...
    if (shouldInsertNewGuess) {
      for (uint256 i = 0; i < currentTopGuesses.length; i++) {
        if (
          currentTopGuesses[i].guesser == address(0) ||
          newGuess.distance < currentTopGuesses[i].distance
        ) {
  ...
  function calculateEuclideanDistanceSquared(
  ...
  function getRevealedChallengeDetails(
    uint256 challengeId
  ) public view returns (GuessDetails[3] memory topGuessesArray, string memory privateImageUrl) {
